...
function bombForEachX(bombs, x, y, noRow, noCol) {
    let j = 0, sign = 1;
    if (getRandom_0toN(2)) {
        j = x - 1; sign = -1;
    }
    for (; j < x && j > -1 && bombs; j += sign) {
        let i = getRandom_0toN(y);
        if (gameState[i][j] == 9) continue;
        if (Math.abs(i - noRow) < 2 && Math.abs(j - noCol) < 2) continue;
        gameState[i][j] = 9;
        bombs--;
    }
    return bombs;
...
function bombForEachY(bombs, x, y, noRow, noCol) {
    let i = 0, sign = 1;
    if (getRandom_0toN(2)) {
        i = y - 1; sign = -1;
    }
...
function createArray(x, y, noRow, noCol) {
    let bombs = x * y;
    if (bombs == 16) bombs = 1;
    ...
    if (x < y) {
        while (bombs = bombForEachY(bombForEachX(bombs, x, y, noRow, noCol), x, y, noRow, noCol));
    }
    else {
        while (bombs = bombForEachX(bombForEachY(bombs, x, y, noRow, noCol), x, y, noRow, noCol));
    }
}
